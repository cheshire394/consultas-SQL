...
SELECT LTRIM(RTRIM( TITULO, '".'),'"') AS TITULO, SUBSTR(TITULO, 1, 3) AS EDITORIAL
FROM MISTEXTOS; 
...
SELECT TRANSLATE(TITULO, '".', ' ') AS TITULO, LOWER(EDITORIAL)
FROM MISTEXTOS; 
...
SELECT INITCAP(LTRIM(RTRIM(TITULO,'".'), '"')) 
FROM MISTEXTOS; 

SELECT INITCAP(TRANSLATE(TITULO, '".',' ')) FROM MISTEXTOS; 
...
SELECT AUTOR || '****' AS AUTOR , EDITORIAL
FROM MISTEXTOS
ORDER BY LENGTH(EDITORIAL)DESC; 

SELECT CONCAT(AUTOR, '***') AS AUTOR, EDITORIAL
FROM MISTEXTOS
...
SELECT SUBSTR(AUTOR,INSTR(AUTOR,',')+2) AS NOMBRE, SUBSTR(AUTOR, 1, INSTR(AUTOR,',')-1) AS APELLIDO
FROM LIBROS; 
...
SELECT SUBSTR(AUTOR, 1, INSTR (AUTOR,' ' )-1) AS APELLIDO, SUBSTR(AUTOR,INSTR(AUTOR, ' ')+1)AS NOMBE
FROM LIBROS; 
...
SELECT SUBSTR(AUTOR, INSTR(AUTOR, ',')+2) AS NOMBRE
FROM LIBROS; 
