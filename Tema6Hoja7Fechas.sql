...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE, FECHA_ALT)/12) * 200 AS PAGA, EMP_NO, ROUND(MONTHS_BETWEEN(SYSDATE, FECHA_ALT)/12) AS AÑOS
FROM EMPLE
WHERE ROUND(MONTHS_BETWEEN(SYSDATE, FECHA_ALT)/12) > 3; 


--7. Mostrar cuantos años lleva trabajando el empleado más antiguo de la empresa.

--RESULTADO CORRECTO
SELECT APELLIDO, FECHA_ALT, ROUND(MONTHS_BETWEEN(SYSDATE, FECHA_ALT)/ 12) AS AÑOS_TRABAJADOS
FROM EMPLE
...
SELECT ROUND(MONTHS_BETWEEN(FECHA_ARREGLO, FECHA_FALLA)*30) AS DIAS, AT.NOM_ATRACCION, Z.NOM_ZONA, Z.PRESUPUESTO
FROM AVERIAS_PARQUE AVE
JOIN ATRACCIONES AT ON AVE.COD_ATRACCION = AT.COD_ATRACCION
JOIN ZONAS Z ON Z.NOM_ZONA = AT.NOM_ZONA --JOIN CON UNA TABLA JOIN
...
SELECT ROUND(FECHA_ARREGLO - FECHA_FALLA) AS DIAS_AVERIADA, AT.NOM_ATRACCION, Z.NOM_ZONA, Z.PRESUPUESTO, AT.COD_ATRACCION
FROM AVERIAS_PARQUE AVE
...
SELECT SUM(COSTE_AVERIA) 
FROM AVERIAS_PARQUE
WHERE TO_CHAR(FECHA_ARREGLO,'HH24:MI') > '18:00';
